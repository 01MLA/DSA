package DSA_implementations.data_structures;

import DSA_implementations.data_structures.Trie.Node;
import DSA_implementations.data_structures.interfaces.IMap;
...
public class TrieMap<K extends CharSequence, V> implements Trie.INodeCreator, IMap<K,V> {
    ...
    public V put(K key, V value) {
        V prev = null;
        Node node = trie.addSequence(key);
    ...
    private boolean validate(Node node, String string, java.util.Set<K> keys) {
    ...
    protected static class TrieMapNode<V> extends Node {

        protected V value = null;

        protected TrieMapNode(Node parent, Character character, boolean type) {
            super(parent, character, type);
        }

        protected TrieMapNode(Node parent, Character character, boolean type, V value) {
            super(parent, character, type);
            this.value = value;
        }
    ...
    private static class JavaCompatibleTrieMap<K extends CharSequence,V> extends java.util.AbstractMap<K,V> {
        ...
        public java.util.Set<Entry<K, V>> entrySet() {
            ...
            if (map.trie!=null && map.trie.root!=null) {
                Node n = map.trie.root;
                levelOrder(n,"",set);
            }
            return set;
        }

        private void levelOrder(Node node, String string, java.util.Set<Entry<K, V>> set) {
            StringBuilder builder = new StringBuilder(string);
            TrieMapNode<V> tmn = null;
            if (node instanceof TrieMapNode) {
                tmn = (TrieMapNode<V>)node;
                if (tmn.character!= Node.SENTINAL) builder.append(tmn.character);
                if (tmn.isWord) {
                    K s = (K)builder.toString();
                    Entry<K, V> entry = new JavaCompatibleMapEntry<K, V>(s, tmn.value);
                    set.add(entry);
                }
            }
